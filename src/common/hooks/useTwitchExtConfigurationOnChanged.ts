...
type ExtConfig = Pick<
  typeof Twitch.ext.configuration,
...
export const deserializeTwitchExtConfiguration = (extConfig: ExtConfig): AppConfiguration | undefined => {
  ...
  try {
    const broadcasterContent = extConfig.broadcaster?.version !== "2"
      ? "{}"
      : extConfig.broadcaster?.content;
  ...
  React.useEffect(() => {
    if (isLocalhost) {
      ...
      setTimeout(() => {
        ...
        handler(createAppConfigurationFromConfigs({
