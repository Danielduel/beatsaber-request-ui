...
type BoundableEvent2EventType<B extends BoundableEvents, E extends HTMLElement> = 
  B extends "onchange"
    ? ChangeEvent<E>
    : never;

export const getGetValueFromEvent = <B extends BoundableEvents, E extends BoundableHTMLElements>(eventToBindTo: B): ((e: BoundableEvent2EventType<B, E>) => string) => {
  switch (eventToBindTo) {
    case "onchange": return (e) => e.target.value;
  }
  return () => "";
}
...
export const useFormField = <E extends BoundableHTMLElements, T>(
  ...
  const sinkEvent = React.useCallback((event: BoundableEvent2EventType<typeof eventToBindTo, E>) => {
